...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        ...
        if title.bytes().len()>50 {panic!("Title cannot be longer than 50 bytes")}
        if description.bytes().len()>500 {panic!("Description cannot be longer than 500 bytes")}      
        if !(["To-Do".into(), "In Progress".into(), "Done".into()].contains(&status)) {panic!("Only `To-Do`, `In Progress`, and `Done` statuses are allowed")}
        Self {
            title,
            description,
            status,
